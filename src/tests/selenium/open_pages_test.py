...
class OpenPagesTest(BasicTest):
    def test_open_slash(self):
        ...
        assert (
            "Welcome to ASM Web IDE" in self.driver.title
        ), "No 'Welcome to ASM Web IDE' in title"
    ...
    def test_open_empty_hexview(self):
        text = ""
        hextext = [
            "3C 4E 6F 20 63 6F 64 65 20 66 6F 72 20 68 65 78",
            "76 69 65 77 21 3E",
        ]
    ...
    def test_open_unexistent_hexview_by_get(self):
        ...
        assert (
            "No such code_id" in self.driver.page_source
        ), "No 'No such code_id' for unexistent code_id"
    ...
    def set_code_and_open_hexview(self, text, hextext_list):
        ...
        assert all(
            [text in self.driver.page_source for text in hextext_list]
        ), f"No '{hextext_list}' in hexview"
